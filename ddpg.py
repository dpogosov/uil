...
class DDPG(object):
    def __init__(self, epoch, net, env_spec, replay_buffer, es, batch_size=32, min_buffer_size=10000, max_path_length=200,
            pi_opimizer=None, q_optimizer=None, pi_lr=1e-3, q_lr=1e-4, sess=None, tau=0.001, gamma=0.99, scale_reward=0.01,
        ...
        if pi_opimizer is None:
            ...
            self.pi_opimizer_fast = tf.train.AdamOptimizer(pi_lr*10.)
        if q_optimizer is None:
            self.q_optimizer = tf.train.AdamOptimizer(q_lr)
            self.q_optimizer_fast = tf.train.AdamOptimizer(q_lr*10.)
        self.sess = sess
        self.tau = tau
        self.gamma = gamma
    ...
    def build(self):
        ...
        self.q_updater_fast = self.q_optimizer_fast.minimize(self.q_loss, var_list=self.net.var_q)
        self.pi_updater = self.pi_opimizer.minimize(self.pi_loss, var_list=self.net.var_pi)
        self.pi_updater_fast = self.pi_opimizer_fast.minimize(self.pi_loss, var_list=self.net.var_pi)
    ...
    def optimize_q(self, batch, fast=False):
        if not self.built:
            self.build()
        ...
        if not fast:
            ...
            self.sess.run(self.q_updater_fast, feed_in)

    def optimize_pi(self, batch, fast=False):
        if not self.built:
            self.build()
        feed_in = {self.states: batch['states'],
                K.learning_phase(): 1}
        if not fast:
            ...
            self.sess.run(self.pi_updater_fast, feed_in)
    ...
    def train_step(self):
        ...
        tru = Controller(vj=0.75, dt=0.05, u_max=2., radius=1., A=0.1)
        ...
        for i in range(self.epoch):
            ...
            for step in range(max_path_length):
                if terminal:
                    break
                if i<100:
                    err, a, dP = tru.point(np.asarray(s[0:2]), dP, self.env.target, magic=75.)
                else:
                    a, agent_info = self.get_action(s, i)
