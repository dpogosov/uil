...
class LSTM():
    ...
    def __init__(self, args, infer=False):
        ...
        #ell = rnn_cell.MultiRNNCell([cell] * args.num_layers, state_is_tuple=False)
        #cell = rnn.MultiRNNCell([cell] * args.num_layers)
    ...
    def sample(self, sess, traj, num=10, full=False):
        '''
        ...
        ret = traj
        ret_var = []#np.array([0, 0])#np.empty(shape=(1, 2))
        ret_mu = []#.empty(shape=(1, 2))
        ...
        for t in range(num):
            ...
            #print('o_mux',o_mux[0])
            #print('A=',ret_mu, 'B=', [[o_mux[0][0], o_muy[0][0]]])
            if not t:
                ret_mu = np.asarray([o_mux[0][0], o_muy[0][0]])
                ret_var = np.asarray([o_sx[0][0], o_sy[0][0]])
            else:
                ret_mu = np.vstack((ret_mu, [o_mux[0][0], o_muy[0][0]]))
                ret_var = np.vstack((ret_var, [o_sx[0][0], o_sy[0][0]]))
            #print(t)
            # Set the current sampled position as the last observed position
            prev_data[0, 0, 0] = next_x
            prev_data[0, 0, 1] = next_y
        if full:
            return ret, ret_mu, ret_var
            #print(ret_mu)
        else:
            return ret
